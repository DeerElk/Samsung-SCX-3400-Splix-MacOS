...
# Flags
CXXFLAGS        += -isysroot $(shell xcrun --show-sdk-path) -I$(shell xcrun --show-sdk-path)/usr/include -Iinclude -Wall
DEBUG_CXXFLAGS  += -DDEBUG -DDUMP_CACHE
OPTIM_CXXFLAGS  += -g

JBIG_DIR        ?=
ifneq ($(strip $(JBIG_DIR)),)
CXXFLAGS        += -I$(JBIG_DIR)
rastertoqpdl_LDFLAGS := $(LDFLAGS) -L$(JBIG_DIR)
else
rastertoqpdl_LDFLAGS := $(LDFLAGS)
endif

rastertoqpdl_LIBS := -lcups -lcupsimage
pstoqpdl_LDFLAGS  := $(LDFLAGS)
pstoqpdl_LIBS     := -lcups -lcupsimage
...
CUPSPPD			?= /usr/share/cups/model
CUPSDRV			?= /usr/share/cups/drv
ifeq ($(ARCHI),Darwin)
